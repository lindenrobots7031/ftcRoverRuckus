...
public class BBGoldAlignDetector extends DogeCVDetector {
    ...
    public DogeCVColorFilter yellowFilter      = new BattleBlazerFilter(lower,upper); //Default Yellow filter
    ...
    public Mat process(Mat input) {
        ...
        for(MatOfPoint cont : contoursYellow){
            ...
            Imgproc.rectangle(displayMat, rect.tl(), rect.br(), new Scalar(0,0,255),3); // Draw rect

            double rectArea = rect.area();

            // If the result is better then the previously tracked one, set this rect as the new best
            if(score < bestDiffrence && rectArea > 4200 && rectArea < 7500){
                bestDiffrence = score;
                bestRect = rect;
            }
        }
        ...
        if(bestRect != null){
            ...
            goldXPos = xPos;
            goldWidth = bestRect.width;
            goldArea = goldWidth * bestRect.height;

            // Draw center point
            Imgproc.circle(displayMat, new Point( xPos, bestRect.y + (bestRect.height / 2)), 5, new Scalar(0,255,0),3);
